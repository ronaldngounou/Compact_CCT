...
class TextTokenizer(nn.Module):
    def __init__(self,
                 kernel_size, stride, padding,
                 pooling_kernel_size=3, pooling_stride=1, pooling_padding=0,
                 embedding_dim=300,
                 n_output_channels=1,
                 activation=None,
                 max_pool=True,
    ...
    def forward_mask(self, mask):
        ...
        new_mask = F.conv1d(
            ...
            self.conv_layers[0].stride[0], self.conv_layers[0].padding[0], groups=1)
        if self.max_pool:
            new_mask = F.max_pool1d(
                new_mask, self.conv_layers[2].kernel_size[0],
                stride = self.conv_layers[2].stride[0])
